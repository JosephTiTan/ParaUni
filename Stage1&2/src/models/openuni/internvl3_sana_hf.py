...
class OpenUniInternVL3SANAHF(BaseModel):
    def __init__(self,
        ...
        self.parscale_n = 28 # 0~28
        self.parscale_aggregate_attn_smoothing = 0.5
        self.agglayer_norm = nn.LayerNorm(self.transformer.config.caption_channels)
    ...
    def text2image_loss(self, data_dict):
        ...
        if self.parscale_n > 1:
            hidden_states = rearrange(hidden_states, "(n_parscale b) s h -> n_parscale b s h", n_parscale=self.parscale_n)
            hidden_states = self.agglayer_norm(hidden_states)
    ...
    def generate(self,
        ...
        # print("see samples", samples.shape)
